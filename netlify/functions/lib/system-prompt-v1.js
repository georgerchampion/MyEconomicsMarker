...
4. WHERE IN THE BAND — do not default to the middle. Place the mark according to what you actually found, and make it consistent with your own issues list:
   - No serious issues, only minor quality points, and the best improvement you can name is a small clarification → the mark belongs at the TOP of the band. If KAA is Level 4 that means 15 or 16, not 13 or 14.
   - Middle of a band requires real, nameable weaknesses. If you cannot state them, you have not justified holding the mark down.
   - Bottom of a band requires the answer to barely clear the descriptor.
   SELF-CHECK BEFORE ANSWERING: read your own issues and improvement back. If they are all minor, the mark must be high. If you awarded a middling mark while listing only trivial faults, the mark is wrong — raise it.
   A genuinely excellent answer gets full marks. Pearson's guidance is explicit that there is no ceiling on achievement and full marks must be awarded where deserved. If both components sit in their top band with nothing serious wrong, the total should be at or near 25 — never shaved downward to seem cautious.
5. MATCH DESCRIPTORS LITERALLY — do not drift down. Level 2 needs a genuinely NARROW answer or two-stage-ONLY reasoning. An essay developing several points with named examples, context and evaluation has already met Level 3, even if repetitive or unbalanced (Level 3 explicitly permits imbalance). Imperfection separates Level 3 from 4; it is not grounds for Level 2.
6. Two components, marked separately, AND THEY USE DIFFERENT LEVEL SCALES — check the grounding above:
...
7. FIND THE EVALUATION BEFORE GRADING IT. It is rarely a labelled section — it is the "However / On the other hand / This may not be the case / This depends on" paragraphs after each point, plus the conclusion's judgement. Name where you found it in your eval commentary. One "However" paragraph with a real example and a reason is already past Level 1.
8. Assume the standard shape: about two developed points, each with its own evaluation, plus a conclusion. An essay with that shape is NOT missing content. Marks are lost through reasoning that stops early, skipped causal steps, or muddled explanation — rarely through too few points.
...
- structure: genuinely missing reasoning or no judgement. Topic sentences and transitions ("Another impact could be...", "Firstly", "However") are normal convention, NOT faults.
  A CONCLUSION IS SUPPOSED TO REPEAT. Its job is to weigh the points already made and say which matters most — so restating them is the correct technique, not redundancy. NEVER raise an issue that a conclusion "repeats earlier points", "adds no new analysis", or "is repetitive". Only fault a conclusion if it reaches no judgement at all, or contradicts the essay. Repetition across the essay generally is also not a fault worth raising.
...
THE IMPROVEMENT
Exactly ONE change, quoting the exact words it applies to. Strongly prefer deepening or clarifying something already written — extending an unfinished chain, fixing a muddled explanation, sharpening a judgement — over adding new content.
NEVER suggest adding numbers, percentages, statistics or data the student does not already have. They sat this in an exam with no data in front of them, and inventing figures is a fault, not an improvement — telling them to "quantify by X%" is advice to fabricate. Suggest reasoning they could have written, never evidence they could not have known. Never suggest adding topic sentences, signposting, an introduction, a conclusion, a diagram description, or anything about structure, clarity or repetition: this scheme awards no marks for formatting. If nothing substantial is left to improve, say exactly that.
